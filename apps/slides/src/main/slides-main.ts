/**
 * MutantcatOffice Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
import { buildPagePptx, parsePageSpec } from '@mutantcatoffice/pipelines/slides'
...
import { getUiLang, normalizeLang, setUiLang } from '@mutantcatoffice/i18n'
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/MutantcatOffice):
...
/** Directory where AI-generated drafts are saved: the configurable default save folder (falls back to <Documents>/MutantcatOffice) */
...
/**
 * Auto-save the draft to <Documents>/MutantcatOffice/<name>.pptx after AI generation completes.
...
export function registerSlidesIpc(): void {
  ...
  // ── Local single-page generation (BYOK): a JSON slide spec written by
  ...
  // Shim over the canonical op (see @mutantcatoffice/pptx-ops): the op owns validation/mutation/journal;
...
async function applyMainProcessProxy(): Promise<void> {
  const setDispatcher = async (proxyUrl: string) => {
    // spawned CLI children do their own fetch and never see the
  ...
  try {
    ...
    // the LLM proxy actually targets
