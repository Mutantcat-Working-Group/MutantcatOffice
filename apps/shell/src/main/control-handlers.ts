...
} from '@mutantcatoffice/cli/control-protocol'
  ...
  return async (req) => {
    ...
    if (req.cmd === 'open') {
      ...
      if (existing) host.activateTab(existing.id)
        ...
        return fail('unsupported', `MutantcatOffice cannot open ${req.path}`)
      }
    ...
    if (!tab) {
      return fail('file_not_open_in_gui', `MutantcatOffice does not have ${req.path} open`, {
