 ...
 * punctuation ride the bundled MutantcatOffice UI Kana JP faces (Noto Sans JP
...
describe('Meiryo UI GO', () => {
  ...
  it('serves kana and JP punctuation from the bundled faces without size-adjust', () => {
    expect(find(KANA_RANGE)?.src).toBe("url('./MutantcatOfficeUIKanaJP-Regular.woff2') format('woff2')")
    expect(find(KANA_RANGE, true)?.src).toBe(
      "url('./MutantcatOfficeUIKanaJP-Bold.woff2') format('woff2')",
    )
...
describe('MutantcatOffice UI Kana JP (Meiryo UI advances, full height)', () => {
  const regular = readWoff2(join(FONTS, 'MutantcatOfficeUIKanaJP-Regular.woff2'))
  const bold = readWoff2(join(FONTS, 'MutantcatOfficeUIKanaJP-Bold.woff2'))
