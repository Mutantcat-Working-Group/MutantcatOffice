...
const sans = readWoff2(join(FONTS, 'MutantcatOfficeSansKR-Regular-subset.woff2'))
const serif = readWoff2(join(FONTS, 'MutantcatOfficeSerifKR-Regular-subset.woff2'))
...
describe('MutantcatOffice Sans KR (Malgun-normalized)', () => {
...
describe('MutantcatOffice Serif KR (Batang-normalized)', () => {
...
describe('MutantcatOffice Che Latin KR (fixed-pitch half-width)', () => {
  const che = readWoff2(join(FONTS, 'MutantcatOfficeCheLatinKR.woff2'))
...
describe('MutantcatOffice Gothic KR (real source metrics, unmodified)', () => {
  const gothicKr = readWoff2(join(FONTS, 'MutantcatOfficeGothicKR-Regular-subset.woff2'))
  ...
  it('keeps every primary font identifier clear of Reserved Font Names', () => {
    ...
    expect(names).toContainEqual({ nameId: 1, value: 'MutantcatOffice Gothic KR' })
    expect(names).toContainEqual({ nameId: 6, value: 'MutantcatOfficeGothicKR-Regular' })
  })
})
