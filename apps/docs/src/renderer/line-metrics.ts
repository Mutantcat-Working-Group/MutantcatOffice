...
import type { DocGrid } from '@mutantcatoffice/docx-engine'
...
const KO_FONT_RE =
  /malgun|맑은|batang|바탕|myeongjo|myungjo|명조|gungsuh|궁서|gulim|굴림|dotum|돋움|nanum|나눔|mutantcatoffice (sans|serif) kr|(noto|source han) (sans|serif)[^,]*\bk(r|orean)?\b/i
...
export const BUNDLED_FONTS = new Set([
  ...
  'MutantcatOffice Gothic KR',
  'MutantcatOffice Poppins',
  'MutantcatOffice Tamil',
  'MutantcatOffice Fullwidth TC',
  'MutantcatOffice Songti SC',
  'Carlito GO',
  'Aptos GO',
  'Aptos Display GO',
  'Century Gothic GO',
...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'MutantcatOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'MutantcatOffice Box Drawing'
  ...
  // 'MutantcatOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
  ...
  if (f.includes('aptos')) {
    ...
    const alias = /narrow|mono|serif/.test(f)
      ...
      : f.includes('display')
        ? 'Aptos Display GO'
        : 'Aptos GO'
    return `${chain(font, alias, 'MutantcatOffice PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  if (SEGOE_UI_TEXT_RE.test(f))
    return `${chain(font, 'Segoe UI GO', 'Noto Sans CJK GO', 'MutantcatOffice PUA Blank')},sans-serif`
  ...
  if (f.includes('poppins')) return `${chain(font, 'MutantcatOffice Poppins', CJK_SANS)},sans-serif`
  ...
  // 'MutantcatOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'MutantcatOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'MutantcatOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
  // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
  const JA_SANS = ['Yu Gothic', 'MutantcatOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
  const JA_SERIF = [
    'Yu Mincho',
    'MutantcatOffice Hiragino Mincho',
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'MutantcatOffice Gothic KR', ...KO_SANS)},sans-serif`
    }
    ...
    if (/(?:batang|gulim|dotum|gungsuh) ?che\b|(?:바탕|굴림|돋움|궁서)체/i.test(nfkc)) {
      ...
      return `${chain(font, 'MutantcatOffice Che Latin KR', ...(cheGungsuh ? ['GungSeo'] : []), ...(cheSerif ? KO_SERIF : KO_SANS))},${cheSerif ? 'serif' : 'sans-serif'}`
    }
  ...
  if (/nyala|ebrima|abyssinica|ethiopic/i.test(nfkc)) {
    return `${chain(font, 'MutantcatOffice Ethiopic')},sans-serif`
  }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'MutantcatOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['MutantcatOffice PUA Blank'] : []
