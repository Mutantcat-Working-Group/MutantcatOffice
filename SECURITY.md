...
[private vulnerability reporting](https://github.com/Mutantcat-Working-Group/MutantcatOffice/security/advisories/new)
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@mutantcatoffice/electron-utils` → `safeExternalUrl`) that parses the URL and
