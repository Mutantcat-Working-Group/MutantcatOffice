 ...
 * when one is configured. BYOK providers answer with bytes; those land in
...
import {
  readGeneratedImage,
  storeGeneratedImage,
} from '@mutantcatoffice/electron-utils/generated-images'
import {
  ResponseTooLargeError,
  fetchRemoteImage,
  readBodyCapped,
} from '@mutantcatoffice/electron-utils/remote-image'
import { fetchWithSsrfGuard } from '@mutantcatoffice/electron-utils/safe-remote-url'
/** 200 MB: enough for a long clip through the Gemini Files API, small enough to hold in memory */
const MAX_MEDIA_BYTES = 200 * 1024 * 1024

/** the only load failure that may hand the request back to the BYOK provider; validation failures never do */
...
export function readAiSettingsFile(path: string): AiSettings {
  return resolveAiSettings(readStoredAiSettingsFile(path), defaultAiSettings())
}

/** The raw stored settings before default sections are merged in; {} when missing or unreadable. */
export function readStoredAiSettingsFile(path: string): Partial<AiSettings> & LegacyAiSettings {
  try {
...
export async function generateImageTool(
  ...
  try {
    if (!byok) return { error: options.notLoggedInError ?? 'No image provider configured' }
...
export async function analyzeMediaTool(
  ...
  try {
    if (!imageByok && !videoByok)
      return { error: options.notLoggedInError ?? 'No media provider configured' }
    ...
    if (!byok) return { error: options.notLoggedInError ?? 'No media provider configured' }
    return {
