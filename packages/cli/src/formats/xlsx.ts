...
import { columnLabel, parseAddress } from '@mutantcatoffice/xlsx-gateway/domain/cell-address'
import {
  assembleWithJsZip,
...
} from '@mutantcatoffice/xlsx-gateway/gateway/xlsx-style-read'
...
async function withSidecar<T>(fn: (client: XlsxSidecarClient) => Promise<T>): Promise<T> {
  ...
  if (!binary) {
    throw new CliError(EXIT.conversion, 'xlsx engine (xlsx-sidecar) not found', {
      hint: 'set XLSX_SIDECAR_PATH or run a packaged MutantcatOffice',
    })
  }
