import type { Block, FieldDisplay } from '@mutantcatoffice/docx-engine'
  ...
  if (blank.length) {
    issues.push(
      issue(
        ...
        'warning',
        blank[0]!,
        `table of contents: ${blank.length} of ${toc.length} entries have no page number; Word computes them on open, or open and save in MutantcatOffice`,
      ),
    )
  }
