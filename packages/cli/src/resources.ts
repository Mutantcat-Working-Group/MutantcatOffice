...
/** How to start the MutantcatOffice GUI: the app binary that hosts this CLI, an installed app, or the dev checkout. */
...
function installedAppBinaries(env: NodeJS.ProcessEnv): string[] {
  switch (process.platform) {
    case 'darwin':
      return [
        '/Applications/MutantcatOffice.app/Contents/MacOS/MutantcatOffice',
        join(homedir(), 'Applications/MutantcatOffice.app/Contents/MacOS/MutantcatOffice'),
      ]
    case 'win32':
      return [
        env.LOCALAPPDATA
          ? join(env.LOCALAPPDATA, 'Programs', 'MutantcatOffice', 'MutantcatOffice.exe')
          : '',
        env.ProgramFiles ? join(env.ProgramFiles, 'MutantcatOffice', 'MutantcatOffice.exe') : '',
      ].filter(Boolean)
    default:
      return ['/opt/MutantcatOffice/genoffice', '/usr/bin/genoffice']
  }
}
