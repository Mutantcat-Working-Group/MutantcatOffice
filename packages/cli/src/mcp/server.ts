...
const ABOUT =
  'MutantcatOffice: create, read, convert, edit and render Office documents locally (docx, xlsx, pptx, pdf, md, html, csv). The app need not be running; render, convert-to-pdf and create_pdf start a hidden MutantcatOffice process for a few seconds.'
const WORKFLOW = [
...
export const INSTRUCTIONS = [
  ABOUT,
  'Paths are absolute, or relative to the working directory the server was started in. Only search, image and media send data off the machine, to the provider configured in MutantcatOffice.',
  ...WORKFLOW,
  'A file open in a MutantcatOffice tab is not written unless force is set. Do not call open unless the user asks to see the file.',
].join('\n')
...
export function remoteInstructions(baseUrl: string): string {
  ...
  return [
    ...
    'Omit out: the file a tool writes comes back in the result as output_url (download it with curl -o) and, when small, as an embedded resource with the bytes. Only search, image and media send data to the cloud provider configured in MutantcatOffice.',
    ...WORKFLOW,
  ].join('\n')
}
...
export function createMcpServer(ctx: McpContext, opts: ServerOptions = {}): McpServer {
  ...
  for (const tool of resolveTools(registry)) {
    // there is no MutantcatOffice window in front of a remote client
