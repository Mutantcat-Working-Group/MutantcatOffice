...
export const openCommand: CommandDef = {
  name: 'open',
  summary:
    'Open a document in the MutantcatOffice app (starts the app if needed); with a target, also select that spot for the user.',
  usage:
  ...
  async run(args, ctx) {
    ...
    if (!endpoint) {
      ...
      if (!target) return { summary: `opening ${path} in MutantcatOffice`, detail: { app: launch } }
      endpoint = await waitForControlEndpoint(ctx.env, APP_START_TIMEOUT_MS)
      if (!endpoint) {
        throw new CliError(
          EXIT.app,
          'MutantcatOffice started but did not publish its control endpoint',
          undefined,
          {
    ...
    return {
      summary: target
        ? `${path}: showing ${describe(target)} in MutantcatOffice`
        : `opened ${path} in MutantcatOffice`,
...
async function spawnApp(path: string, ctx: CommandContext): Promise<string> {
  ...
  if (!launch) {
    throw new CliError(EXIT.app, 'MutantcatOffice app not found', { hint: 'set GENOFFICE_APP_BIN' })
  }
  const env = { ...ctx.env }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once('error', (err) =>
      reject(new CliError(EXIT.app, `failed to start MutantcatOffice: ${err.message}`)),
    )
    child.once('spawn', () => {
      child.unref()
      resolve()
