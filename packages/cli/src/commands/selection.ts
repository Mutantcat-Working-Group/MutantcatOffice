...
export const selectionCommand: CommandDef = {
  name: 'selection',
  summary:
    "What the user has selected in the MutantcatOffice editor showing this file (the user's own pointer for 'this one', 'here').",
  usage: 'selection <file> --json',
  async run(args, ctx) {
    const path = resolveInput(args.positionals[0], ctx)
    const endpoint = controlEndpoint(ctx.env)
    if (!endpoint) {
      throw new CliError(EXIT.app, 'MutantcatOffice is not running', undefined, {
