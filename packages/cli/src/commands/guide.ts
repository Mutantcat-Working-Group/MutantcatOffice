...
export const guideCommand: CommandDef = {
  ...
  async run(args) {
    ...
    if (domain === 'slides' && (topic === 'design' || topic === 'spec')) {
      const { SLIDES_GUIDES } = await import('@mutantcatoffice/pipelines/slides')
...
async function load(
  ...
  const docs = await import('@mutantcatoffice/pptx-ops/op-docs')
...
type SlidesOpDocs = typeof import('@mutantcatoffice/pptx-ops/op-docs')
