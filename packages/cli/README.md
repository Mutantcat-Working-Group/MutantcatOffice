...
`genoffice` is the MutantcatOffice command line. It exposes the suite's document engines
...
genoffice capabilities --json                        # which cloud features MutantcatOffice has configured (no network call)
...
runs on the app's Node runtime like every other command, and MutantcatOffice itself
...
- **macOS**: the app tries to symlink `/usr/local/bin/genoffice` (or `/opt/homebrew/bin/genoffice`) on every launch until one succeeds. If neither directory is writable it stays silent; run `genoffice install-cli` from the launcher, or `sudo mkdir -p /usr/local/bin && sudo ln -sf "/Applications/MutantcatOffice.app/Contents/Resources/cli/genoffice" /usr/local/bin/genoffice`.
...
- `src/formats/` — thin adapters over `@mutantcatoffice/pdf2docx`, the xlsx sidecar
...
- `bin/genoffice`, `bin/genoffice.cmd` — launchers copied next to `mutantcatoffice.cjs` in the
  packaged app.
...
- A file the running MutantcatOffice shell has open in a tab is not rewritten in place
...
otherwise Mutantcat AI is the default when signed in (`~/.genoffice/auth.json`)
...
(`MutantcatOffice/ai-settings.json` in the platform config directory, override with
...
```
npm run build -w @mutantcatoffice/cli       # esbuild → dist/mutantcatoffice.cjs
...
Word → HTML, HTML → Word, `create --type pdf`) run inside the MutantcatOffice binary
