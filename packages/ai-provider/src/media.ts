 ...
 * that capability and is usable; anything else falls back to the default
 * provider so a half-filled setup degrades to a working default.
 */
export function activeMediaProvider(
  ...
  if (!media) return AI_MEDIA_PROVIDERS[0]!.id
  const id =
    capability === 'image'
      ? media.imageProvider
      : capability === 'video'
  ...
  if (!id) return AI_MEDIA_PROVIDERS[0]!.id
  const meta = getMediaProviderMeta(id)
  if (!meta || !providerHasCapability(meta, capability)) return AI_MEDIA_PROVIDERS[0]!.id
  if (!mediaConfigUsable(meta, media.providers?.[id])) return AI_MEDIA_PROVIDERS[0]!.id
  return id
}

/** the active BYOK config for one capability, or null when it runs through the default provider */
...
/** live predicate for the analyze_media tool: image or video analysis is reachable */
