...
describe('chatForProvider', () => {
  ...
  it('anthropic: replaces an HTML error body with a readable note', async () => {
    const html =
      '<!doctype html>\n<html>\n<head><title>Mutantcat AI</title></head><body>app shell</body></html>'
  ...
  it('anthropic: routes by model prefix to the proxy endpoints', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ content: [{ type: 'text', text: 'ok' }] }))
    vi.stubGlobal('fetch', fetchMock)
    await chatForProvider('anthropic', { apiKey: 'codex-k', model: 'claude-opus-4-7' }, 'sys', 'hi')
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.anthropic.com/v1/messages',
      expect.anything(),
    )
    fetchMock.mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'ok' } }] }))
    await chatForProvider('openai', { apiKey: 'codex-k', model: 'gpt-5.2' }, 'sys', 'hi')
  ...
  it('never stamps X-Agent-Type on vendor calls', async () => {
    const fetchMock = vi
      .fn()
      .mockImplementation(async () => jsonResponse({ content: [{ type: 'text', text: 'ok' }] }))
    vi.stubGlobal('fetch', fetchMock)
    await chatForProvider('anthropic', { apiKey: 'codex-k', model: 'claude-opus-4-7' }, 'sys', 'hi')
    await chatForProvider('openai', { apiKey: 'k', model: 'gpt-5.2' }, 'sys', 'hi')
    for (const call of fetchMock.mock.calls) {
      const [, init] = call as unknown as [string, RequestInit]
      expect((init.headers as Record<string, string>)['X-Agent-Type']).toBeUndefined()
    }
  })
