...
describe('provider model catalog', () => {
  it('offers DeepSeek V4.1 Flash directly under the same versioned name as the pool', () => {
    const codex = AI_PROVIDERS.find((provider) => provider.id === 'codex')!
    ...
    expect(codex.models).not.toContain('deep-seek-v4-flash')
    expect(codex.models).not.toContain('deep-seek-v4-flash-vision-exp-openrouter')
  })
...
describe('activeProvider', () => {
  ...
  it('codex never requires a key (injected from the gsk login at request time)', () => {
    const settings = defaultAiSettings()
    settings.provider = 'codex'
    expect(activeProvider(settings)).toBe('codex')
  })
